    ...
    public class FingersZoomPanCameraComponentScript : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            if (_camera.orthographic)
            {
                ...
                float leftOverlap = Mathf.Max(0.0f, visibleAreaBounds.min.x - cameraVisibleArea.min.x);
                float rightOverlap = Mathf.Min(0.0f, visibleAreaBounds.max.x - cameraVisibleArea.max.x);
                float topOverlap = Mathf.Min(0.0f, visibleAreaBounds.max.y - cameraVisibleArea.max.y);
                float bottomOverlap = Mathf.Max(0.0f, visibleAreaBounds.min.y - cameraVisibleArea.min.y);
        ...
        private void Gesture_Updated(DigitalRubyShared.GestureRecognizer gesture)
        {
            ...
            float scale = 1.0f + (1.0f - ScaleGesture.ScaleMultiplier)*0.1f;

            
            if (_camera.orthographic)
            {
                PostSize = _camera.orthographicSize;
                float newOrthographicSize = Mathf.Clamp(_camera.orthographicSize * scale, 3.0f, 20f);
                ...
                if (leftOverlap > 0f || rightOverlap < 0f || bottomOverlap > 0f ||  topOverlap < 0f)
                {
                    _camera.orthographicSize = PostSize;
                }
                else
                    _camera.orthographicSize = newOrthographicSize;
            }
